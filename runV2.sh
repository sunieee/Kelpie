...
for model in complex conve transe; do
    for dataset in FB15k WN18 FB15k-237 WN18RR; do
        ...
        CUDA_VISIBLE_DEVICES=1 python3 verify.py --dataset "$dataset" --model "$model" --metric score --topN 4 --filter head > "${output_dir}/verify_score_h4.log" 2>&1 &
        # 等待这两个进程执行完
        wait
    done
done
...
CUDA_VISIBLE_DEVICES=1 python3 verify.py --dataset FB15k --model complex --metric score --topN 4 --filter head > "out/complex_FB15k/verify_score_h4.log"